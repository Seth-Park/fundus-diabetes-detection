...
@click.option('--load_weights', default=None,
              show_default=True, help='Path or name of the weights')
def main(model, datadir, label, save_weights, load_weights):
    ...
    if load_weights is not None:
        net.load_params_from(load_weights)

    print("Training initiated...")
    net.fit(X, y)
    net.save_params_to(os.path.join(save_weights, 'second_stage_model'))
